...
use App\Services\Reports\LaporanKehadiran as LaporanKehadiranService;

class LaporanKehadiran extends Controller
{
  public function index(Request $request)
  {
    ...
    $data = LaporanKehadiranService::index($request);
    ...
    if ($request->input('export') === 'xlsx') {
      ...
      foreach ($data['data'] as $i => $d) {
        ...
        foreach ($d['absensi'] as $absensi)
          $row[] = (int) $absensi['kehadiran'];
        $row[] = $d['total_kehadiran'];
      ...
      $filename = 'Laporan Kehadiran - ' . $cabang->kode_cabang . ' - ' . $cabang->cabang . '.xlsx';
