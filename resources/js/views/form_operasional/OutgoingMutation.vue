<template>
  <transition name="fade" mode="out-in">
    <div class="row mt-5" v-if="$route.name === 'formOperasional.outgoingMutation'">
                  ...
                  <div class="col-12">
                    <div class="form-group">
                      <div class="input-group">
                        ...
                        <select class="form-control" v-model="query.outgoing_mutation.cabang_id" @change="queryData">
                      ...
                      <tr v-for="(outgoing_mutation, i) in data.outgoing_mutation">
                        ...
                        <td>
                          ...
                          <router-link class="badge badge-warning"
                            :to="{ name: 'formOperasional.outgoingMutation.update', params: { id: outgoing_mutation.id } }"
                            v-if="
                              $access('formOperasional.outgoingMutation', 'update') && (
                                $access('formOperasional.outgoingMutation.update', 'timeFree') ||
                                $moment($moment(query.outgoing_mutation.tanggal_form)).isBetween(
                                  $moment(utils.date).subtract($access('formOperasional.outgoingMutation.update', 'dateMin')).format('YYYY-MM-DD'),
                                  $moment(utils.date).add($access('formOperasional.outgoingMutation.update', 'dateMax')).format('YYYY-MM-DD'),
                                  undefined,
                                  '[]'
                                )
                          ...
                          <a class="badge badge-danger"
                            ...
                            href="#"
                            v-if="
                              $access('formOperasional.outgoingMutation', 'destroy') && (
                                $access('formOperasional.outgoingMutation.destroy', 'timeFree') ||
                                $moment($moment(query.outgoing_mutation.tanggal_form)).isBetween(
                                  $moment(utils.date).subtract($access('formOperasional.outgoingMutation.destroy', 'dateMin')).format('YYYY-MM-DD'),
                                  $moment(utils.date).add($access('formOperasional.outgoingMutation.destroy', 'dateMax')).format('YYYY-MM-DD'),
                                  undefined,
                                  '[]'
                                )
                              ) && outgoing_mutation.approve == 0
      ...
      <div class="modal fade"
        data-entity="outgoingMutation"
        data-method="approve"
        data-backdrop="static"
        data-keyboard="false"
        tabindex="-1"
        v-if="
          $access('formOperasional.outgoingMutation', 'approve') && (
            $access('formOperasional.outgoingMutation.approve', 'timeFree') ||
            $moment($moment(query.outgoing_mutation.tanggal_form)).isBetween(
              $moment().subtract($access('formOperasional.outgoingMutation.approve', 'dateMin')).format('YYYY-MM-DD'),
              $moment().add($access('formOperasional.outgoingMutation.approve', 'dateMax')).format('YYYY-MM-DD'),
              undefined,
              '[]'
            )
          )
      ...
      <div class="modal fade"
        ...
        v-if="
          $access('formOperasional.outgoingMutation', 'destroy') && (
            $access('formOperasional.outgoingMutation.destroy', 'timeFree') ||
            $moment($moment(query.outgoing_mutation.tanggal_form)).isBetween(
              $moment().subtract($access('formOperasional.outgoingMutation.destroy', 'dateMin')).format('YYYY-MM-DD'),
              $moment().add($access('formOperasional.outgoingMutation.destroy', 'dateMax')).format('YYYY-MM-DD'),
              undefined,
              '[]'
            )
          )
...
export default {
  ...
  methods: {
     ...
     */
    fetchMainData() {
      return this.$axios.get('/ajax/v1/form_operasional/outgoing_mutation/cabang_harian', { params: this.query.outgoing_mutation })
    },
    fetchCabang() {
