function ParameterQualityControl(c)
{
  let obj = {
    ...
    ajax: {
      get: (id) =>
        $.ajax({
          method: 'get',
          url: baseUrl.url('/operasional/master/quality_control/parameter_quality_control/get'),
          data: {id: id}
        }),
      search: (d) =>
        $.ajax({
          method: 'get',
          url: baseUrl.url('/operasional/master/quality_control/parameter_quality_control/search'),
          data: d
        }),
      post: (d) =>
        $.ajax({
          method: 'post',
          url: baseUrl.url('/operasional/master/quality_control/parameter_quality_control/post'),
          data: d
        }),
      put: (d) =>
        $.ajax({
          method: 'put',
          url: baseUrl.url('/operasional/master/quality_control/parameter_quality_control/put'),
          data: d
        }),
    },
    $: {
      modal: {
        tambah: modsel('parameterQualityControl', 'tambah'),
        ubah: modsel('parameterQualityControl', 'ubah')
      },
      table: tbsel('parameterQualityControl'),
      search: scsel()
    },
    ...
    load: () => {
      ...
      if (history.pushState) {
        history.pushState(
          null,
          null,
          baseUrl.url(`/operasional/master/quality_control/detail/${obj.qualityControlID}${params}`)
        );
      }
      obj.ajax.search({quality_control_id: obj.qualityControlID})
        ...
        .done((r) => {
          ...
          r.data.forEach((item, index) => obj.$.table.find(tbysel('dataWrapper', true)).append(`
            <tr>
              <td>${index + 1}</td>
              <td>${item.parameter_quality_control}</td>
              <td>${item.opsi_parameter_quality_control.length}</td>
              <td>
                <a href="${baseUrl.url('/operasional/master/quality_control/detail/')}${obj.qualityControlID}/${item.id}" class="badge badge-info">Lihat Opsi Parameter</a>
                <a href="#" class="badge badge-warning" data-toggle="modal" data-target=".modal[data-entity='parameterQualityControl'][data-method='ubah']" data-id="${item.id}">Ubah</a>
                <a href="#" class="badge badge-danger" data-toggle="modal" data-target=".modal[data-entity='parameterQualityControl'][data-method='hapus']" data-id="${item.id}">Hapus</a>
              </td>
            </tr>
          `));  
          pageSpinner.stop();
