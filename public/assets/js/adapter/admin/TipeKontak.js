function TipeKontak()
{
  let obj = {
    ...
    $: {
      modal: {
        tambah: modsel('tipeKontak', 'tambah'),
        ubah: modsel('tipeKontak', 'ubah')
      },
      table: tbsel('tipeKontak'),
      search: scsel()
    },
    ...
    load: () => {
      ...
      if (history.pushState) {
        history.pushState(
          null,
          null,
          baseUrl.url(`/hrd/master/tipe_kontak${params}`)
        );
      }
        ...
        .done((r) => {
          ...
          r.data.forEach((item, index) => obj.$.table.find(tbysel('dataWrapper', true)).append(`
            <tr>
              ...
              <td>
                <a href="#" class="badge badge-warning" data-toggle="modal" data-target=".modal[data-entity='tipeKontak'][data-method='ubah']" data-id="${item.id}">Ubah</a>
                <a href="#" class="badge badge-danger" data-toggle="modal" data-target=".modal[data-entity='tipeKontak'][data-method='hapus']" data-id="${item.id}">Hapus</a>
              </td>
            </tr>
          `));  
          pageSpinner.stop();
