  ...
  let obj = {
    ...
    ajax: {
      get: (id) =>
        $.ajax({
          method: 'get',
          url: baseUrl.url('/operasional/master/kegiatan_kebersihan/get'),
          data: {id: id}
        }),
      search: (d) =>
        $.ajax({
          method: 'get',
          url: baseUrl.url('/operasional/master/kegiatan_kebersihan/search'),
          data: d
        }),
      post: (d) =>
        $.ajax({
          method: 'post',
          url: baseUrl.url('/operasional/master/kegiatan_kebersihan/post'),
          data: d
        }),
      put: (d) =>
        $.ajax({
          method: 'put',
          url: baseUrl.url('/operasional/master/kegiatan_kebersihan/put'),
          data: d
        }),
    },
    $: {
      modal: {
        tambah: modsel('kegiatanKebersihan', 'tambah'),
        ubah: modsel('kegiatanKebersihan', 'ubah')
      },
      table: tbsel('kegiatanKebersihan'),
      search: scsel()
    },
    ...
    load: () => {
      ...
      if (history.pushState) {
        history.pushState(
          null,
          null,
          baseUrl.url(`/operasional/master/kegiatan_kebersihan${params}`)
        );
      }
      obj.ajax.search(obj.query)
        ...
        .done((r) => {
          ...
          r.data.forEach((item, index) => obj.$.table.find(tbysel('dataWrapper', true)).append(`
            <tr>
              <td>${index + 1}</td>
              <td>${item.kegiatan_kebersihan}</td>
              <td>
                <a href="#" class="badge badge-warning" data-toggle="modal" data-target=".modal[data-entity='kegiatanKebersihan'][data-method='ubah']" data-id="${item.id}">Ubah</a>
                <a href="#" class="badge badge-danger" data-toggle="modal" data-target=".modal[data-entity='kegiatanKebersihan'][data-method='hapus']" data-id="${item.id}">Hapus</a>
              </td>
            </tr>
          `));  
          pageSpinner.stop();
