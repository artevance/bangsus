function Divisi()
{
  let obj = {
    ...
    $: {
      modal: {
        tambah: modsel('divisi', 'tambah'),
        ubah: modsel('divisi', 'ubah')
      },
      table: tbsel('divisi'),
      search: scsel()
    },
    ...
    load: () => {
      ...
      if (history.pushState) {
        history.pushState(
          null,
          null,
          baseUrl.url(`/hrd/master/divisi${params}`)
        );
      }
        ...
        .done((r) => {
          ...
          r.data.forEach((item, index) => obj.$.table.find(tbysel('dataWrapper', true)).append(`
            <tr>
              ...
              <td>${item.divisi}</td>
              <td>
                <a href="#" class="badge badge-warning" data-toggle="modal" data-target=".modal[data-entity='divisi'][data-method='ubah']" data-id="${item.id}">Ubah</a>
                <a href="#" class="badge badge-danger" data-toggle="modal" data-target=".modal[data-entity='divisi'][data-method='hapus']" data-id="${item.id}">Hapus</a>
              </td>
            </tr>
          `));  
          pageSpinner.stop();
