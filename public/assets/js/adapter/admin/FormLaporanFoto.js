function FormLaporanFoto()
{
  let obj = {
    query: {},
    rel: {
      cabang: Cabang(),
      kelompokLaporanFoto: KelompokLaporanFoto(),
    ...
    ajax: {
      get: (id) =>
        $.ajax({
          method: 'get',
          url: baseUrl.url('/operasional/form_laporan_foto/get'),
          data: {id: id}
        }),
      cabangHarian: (d) =>
        $.ajax({
          method: 'get',
          url: baseUrl.url('/operasional/form_laporan_foto/cabang_harian'),
          data: d
        }),
      cabangKelompokHarian: (d) =>
        $.ajax({
          method: 'get',
          url: baseUrl.url('/operasional/form_laporan_foto/cabang_kelompok_harian'),
          data: d
        }),
      post: (d) =>
        $.ajax({
      ...
      delete: (d) =>
        $.ajax({
          method: 'delete',
          url: baseUrl.url('/operasional/form_laporan_foto/delete'),
          data: d
        }),
    },
    $: {
      modal: {
        tambah: modsel('formLaporanFoto', 'tambah'),
        ubah: modsel('formLaporanFoto', 'ubah'),
        hapus: modsel('formLaporanFoto', 'hapus')
      },
      table: tbsel('formLaporanFoto'),
      search: scsel()
    },
    ...
    load: () => {
      ...
      if (history.pushState) {
        history.pushState(
          null,
          null,
          baseUrl.url(`/operasional/form_laporan_foto${params}`)
        );
      }
      obj.ajax.cabangHarian(obj.query)
        ...
        .done((r) => {
          ...
          r.data.forEach((item, index) => {
            let aksi = ``;
            aksi += `
              <a href="#" class="badge badge-warning" data-toggle="modal" data-target=".modal[data-entity='formLaporanFoto'][data-method='ubah']" data-id="${item.id}">Ubah</a>
              <a href="#" class="badge badge-danger" data-toggle="modal" data-target=".modal[data-entity='formLaporanFoto'][data-method='hapus']" data-id="${item.id}">Hapus</a>
            `;
            if (item.tidak_kirim == 0) {
              obj.$.table.find(tbysel('dataWrapper', true)).append(`
                <tr>
                  ...
                  <td>${item.jam}</td>
                  <td>${item.kelompok_laporan_foto != null ? item.kelompok_laporan_foto.kelompok_laporan_foto : '-'}</td>
                  <td>
    ...
    responsiveContract: () => {
      ...
      obj.$.modal.tambah.on('show.bs.modal', (e) => {
        ...
        obj.rel.kelompokLaporanFoto.ajax.search()
          .fail((r) => console.log(r))
          .done((r) => {
            console.log(r);
            $(e.currentTarget).find('form').find('[name="kelompok_laporan_foto_id"]').empty().append('<option value="null">-- Pilih Kelompok Foto --</option>');
            r.data.forEach((item, index) => $(e.currentTarget).find('form').find('[name="kelompok_laporan_foto_id"]').append(`
              <option value="${item.id}">${item.kelompok_laporan_foto}</option>
            `))
          });
      });
        ...
        obj.ajax.get($(e.relatedTarget).data('id'))
          ...
          .done((r) => {
            ...
            obj.rel.kelompokLaporanFoto.ajax.search()
              ...
              .done((re) => {
                console.log(re);
                $(e.currentTarget).find('form').find('[name="kelompok_laporan_foto_id"]').empty().append('<option>-- Pilih Kelompok Foto --</option>');
                re.data.forEach((item, index) => $(e.currentTarget).find('form').find('[name="kelompok_laporan_foto_id"]').append(`
                  <option value="${item.id}">${item.kelompok_laporan_foto}</option>
                `));
                $(e.currentTarget).find('form').find('[name="kelompok_laporan_foto_id"]').val(r.data.kelompok_laporan_foto_id);
              });
