  ...
  let obj = {
    ...
    ajax: {
      get: (id) =>
        $.ajax({
          method: 'get',
          url: baseUrl.url('/operasional/form_c/produksi/lpg/get'),
          data: {id: id}
        }),
      cabangHarian: (d) =>
        $.ajax({
          method: 'get',
          url: baseUrl.url('/operasional/form_c/produksi/lpg/cabang_harian'),
          data: d
        }),
      post: (d) =>
        $.ajax({
          method: 'post',
          url: baseUrl.url('/operasional/form_c/produksi/lpg/post'),
          data: d
        }),
      put: (d) =>
        $.ajax({
          method: 'put',
          url: baseUrl.url('/operasional/form_c/produksi/lpg/put'),
          data: d
        }),
      delete: (d) =>
        $.ajax({
          method: 'delete',
          url: baseUrl.url('/operasional/form_c/produksi/lpg/delete'),
          data: d
        }),
    },
    $: {
      modal: {
        tambah: modsel('formLPG', 'tambah'),
        ubah: modsel('formLPG', 'ubah'),
        hapus: modsel('formLPG', 'hapus')
      },
      table: tbsel('formLPG'),
      search: scsel()
    },
    ...
    load: () => {
      ...
      if (history.pushState) {
        history.pushState(
          null,
          null,
          baseUrl.url(`/operasional/form_c/produksi/lpg${params}`)
        );
      }
          ...
          r.data.forEach((item, index) => {
            obj.$.table.find(tbysel('dataWrapper', true)).append(`
              <tr>
                ...
                <td>${item.tipe_proses_lpg != null ? item.tipe_proses_lpg.tipe_proses_lpg : '-'}</td>
                <td>
                  <a href="#" class="badge badge-warning" data-toggle="modal" data-target=".modal[data-entity='formLPG'][data-method='ubah']" data-id="${item.id}">Ubah</a>
                  <a href="#" class="badge badge-danger" data-toggle="modal" data-target=".modal[data-entity='formLPG'][data-method='hapus']" data-id="${item.id}">Hapus</a>
                </td>
              </tr>
            `)}
          );  
    ...
    responsiveContract: () => {
      ...
      obj.$.modal.tambah.on('show.bs.modal', (e) => {
        ...
        obj.rel.tipeProsesLPG.ajax.search()
          .fail((r) => console.log(r))
          .done((r) => {
            console.log(r);
            $(e.currentTarget).find('form').find('[name="tipe_proses_lpg_id"]').empty().append('<option value="null">-- Pilih Tipe Proses LPG --</option>');
            r.data.forEach((item, index) => $(e.currentTarget).find('form').find('[name="tipe_proses_lpg_id"]').append(`
              <option value="${item.id}">${item.tipe_proses_lpg}</option>
            `))
          });
        ...
        obj.ajax.get($(e.relatedTarget).data('id'))
          ...
          .done((r) => {
            ...
            obj.rel.tipeProsesLPG.ajax.search()
              ...
              .done((re) => {
                console.log(re);
                $(e.currentTarget).find('form').find('[name="tipe_proses_lpg_id"]').empty().append('<option>-- Pilih Tipe Proses LPG --</option>');
                re.data.forEach((item, index) => $(e.currentTarget).find('form').find('[name="tipe_proses_lpg_id"]').append(`
                  <option value="${item.id}">${item.tipe_proses_lpg}</option>
                `));
                $(e.currentTarget).find('form').find('[name="tipe_proses_lpg_id"]').val(r.data.tipe_proses_lpg_id);
              });
            obj.rel.satuan.ajax.search()
              .fail((re) => console.log(re))
              .done((re) => {
