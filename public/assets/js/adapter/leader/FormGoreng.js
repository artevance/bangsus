  ...
  let obj = {
    ...
    ajax: {
      get: (id) =>
        $.ajax({
          method: 'get',
          url: baseUrl.url('/operasional/form_c/produksi/goreng/get'),
          data: {id: id}
        }),
      cabangHarian: (d) =>
        $.ajax({
          method: 'get',
          url: baseUrl.url('/operasional/form_c/produksi/goreng/cabang_harian'),
          data: d
        }),
      post: (d) =>
        $.ajax({
          method: 'post',
          url: baseUrl.url('/operasional/form_c/produksi/goreng/post'),
          data: d
        }),
      put: (d) =>
        $.ajax({
          method: 'put',
          url: baseUrl.url('/operasional/form_c/produksi/goreng/put'),
          data: d
        }),
      delete: (d) =>
        $.ajax({
          method: 'delete',
          url: baseUrl.url('/operasional/form_c/produksi/goreng/delete'),
          data: d
        }),
    },
    $: {
      modal: {
        tambah: modsel('formGoreng', 'tambah'),
        ubah: modsel('formGoreng', 'ubah'),
        hapus: modsel('formGoreng', 'hapus')
      },
      table: tbsel('formGoreng'),
      search: scsel()
    },
    ...
    load: () => {
      ...
      if (history.pushState) {
        history.pushState(
          null,
          null,
          baseUrl.url(`/operasional/form_c/produksi/goreng${params}`)
        );
      }
          ...
          r.data.forEach((item, index) => {
            obj.$.table.find(tbysel('dataWrapper', true)).append(`
              <tr>
                ...
                <td>${item.jam}</td>
                <td>${item.item_goreng.item_goreng}</td>
                <td>${item.qty}</td>
                <td>${item.satuan.satuan}</td>
                <td>${item.supplier != null ? item.supplier.supplier : '-'}</td>
                <td>
                  <a href="#" class="badge badge-warning" data-toggle="modal" data-target=".modal[data-entity='formGoreng'][data-method='ubah']" data-id="${item.id}">Ubah</a>
                  <a href="#" class="badge badge-danger" data-toggle="modal" data-target=".modal[data-entity='formGoreng'][data-method='hapus']" data-id="${item.id}">Hapus</a>
                </td>
              </tr>
            `)}
          );  
    ...
    responsiveContract: () => {
      ...
      obj.$.modal.tambah.on('show.bs.modal', (e) => {
        ...
        obj.rel.itemGoreng.ajax.search()
          .fail((r) => console.log(r))
          .done((r) => {
            console.log(r);
            $(e.currentTarget).find('form').find('[name="item_goreng_id"]').empty().append('<option value="null">-- Pilih Item Goreng --</option>');
            r.data.forEach((item, index) => $(e.currentTarget).find('form').find('[name="item_goreng_id"]').append(`
              <option value="${item.id}">${item.item_goreng}</option>
            `))
          });
      });
        ...
        obj.ajax.get($(e.relatedTarget).data('id'))
          ...
          .done((r) => {
            ...
            obj.rel.itemGoreng.ajax.search()
              ...
              .done((re) => {
                console.log(re);
                $(e.currentTarget).find('form').find('[name="item_goreng_id"]').empty().append('<option>-- Pilih Item Goreng --</option>');
                re.data.forEach((item, index) => $(e.currentTarget).find('form').find('[name="item_goreng_id"]').append(`
                  <option value="${item.id}">${item.item_goreng}</option>
                `));
                $(e.currentTarget).find('form').find('[name="item_goreng_id"]').val(r.data.item_goreng_id);
              });
