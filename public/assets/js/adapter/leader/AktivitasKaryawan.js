  ...
  let obj = {
    ...
    ajax: {
      get: (id) =>
        $.ajax({
          method: 'get',
          url: baseUrl.url('/operasional/master/aktivitas_karyawan/get'),
          data: {id: id}
        }),
      search: (d) =>
        $.ajax({
          method: 'get',
          url: baseUrl.url('/operasional/master/aktivitas_karyawan/search'),
          data: d
        }),
      post: (d) =>
        $.ajax({
          method: 'post',
          url: baseUrl.url('/operasional/master/aktivitas_karyawan/post'),
          data: d
        }),
      put: (d) =>
        $.ajax({
          method: 'put',
          url: baseUrl.url('/operasional/master/aktivitas_karyawan/put'),
          data: d
        }),
    },
    $: {
      modal: {
        tambah: modsel('aktivitasKaryawan', 'tambah'),
        ubah: modsel('aktivitasKaryawan', 'ubah')
      },
      table: tbsel('aktivitasKaryawan'),
      search: scsel()
    },
    ...
    load: () => {
      ...
      if (history.pushState) {
        history.pushState(
          null,
          null,
          baseUrl.url(`/operasional/master/aktivitas_karyawan${params}`)
        );
      }
      obj.ajax.search(obj.query)
        ...
        .done((r) => {
          ...
          r.data.forEach((item, index) => obj.$.table.find(tbysel('dataWrapper', true)).append(`
            <tr>
              <td>${index + 1}</td>
              <td>${item.aktivitas_karyawan}</td>
              <td>
                <a href="#" class="badge badge-warning" data-toggle="modal" data-target=".modal[data-entity='aktivitasKaryawan'][data-method='ubah']" data-id="${item.id}">Ubah</a>
                <a href="#" class="badge badge-danger" data-toggle="modal" data-target=".modal[data-entity='aktivitasKaryawan'][data-method='hapus']" data-id="${item.id}">Hapus</a>
              </td>
            </tr>
          `));  
          pageSpinner.stop();
