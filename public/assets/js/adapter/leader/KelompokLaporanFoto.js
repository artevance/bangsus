  ...
  let obj = {
    ...
    ajax: {
      get: (id) =>
        $.ajax({
          method: 'get',
          url: baseUrl.url('/operasional/master/form_laporan_foto/kelompok_laporan_foto/get'),
          data: {id: id}
        }),
      search: (d) =>
        $.ajax({
          method: 'get',
          url: baseUrl.url('/operasional/master/form_laporan_foto/kelompok_laporan_foto/search'),
          data: d
        }),
      post: (d) =>
        $.ajax({
          method: 'post',
          url: baseUrl.url('/operasional/master/form_laporan_foto/kelompok_laporan_foto/post'),
          data: d
        }),
      put: (d) =>
        $.ajax({
          method: 'put',
          url: baseUrl.url('/operasional/master/form_laporan_foto/kelompok_laporan_foto/put'),
          data: d
        }),
    },
    $: {
      modal: {
        tambah: modsel('kelompokLaporanFoto', 'tambah'),
        ubah: modsel('kelompokLaporanFoto', 'ubah')
      },
      table: tbsel('kelompokLaporanFoto'),
      search: scsel()
    },
    ...
    load: () => {
      ...
      if (history.pushState) {
        history.pushState(
          null,
          null,
          baseUrl.url(`/operasional/master/form_laporan_foto${params}`)
        );
      }
      obj.ajax.search(obj.query)
        ...
        .done((r) => {
          ...
          r.data.forEach((item, index) => {
            ...
            if (item.master == 0) {
              aksi += `
                <a href="#" class="badge badge-warning" data-toggle="modal" data-target=".modal[data-entity='kelompokLaporanFoto'][data-method='ubah']" data-id="${item.id}">Ubah</a>
            ...
            obj.$.table.find(tbysel('dataWrapper', true)).append(`
              <tr>
                <td>${index + 1}</td>
                <td>${item.kelompok_laporan_foto}</td>
              </tr>
            `)}
          );  
          pageSpinner.stop();
