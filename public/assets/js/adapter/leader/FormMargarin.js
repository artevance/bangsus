function FormMargarin()
{
  let obj = {
    query: {},
    rel: {
      cabang: Cabang(),
      tipeProsesMargarin: TipeProsesMargarin(),
    ...
    ajax: {
      get: (id) =>
        $.ajax({
          method: 'get',
          url: baseUrl.url('/operasional/form_c/produksi/margarin/get'),
          data: {id: id}
        }),
      cabangHarian: (d) =>
        $.ajax({
          method: 'get',
          url: baseUrl.url('/operasional/form_c/produksi/margarin/cabang_harian'),
          data: d
        }),
      post: (d) =>
        $.ajax({
          method: 'post',
          url: baseUrl.url('/operasional/form_c/produksi/margarin/post'),
          data: d
        }),
      put: (d) =>
        $.ajax({
          method: 'put',
          url: baseUrl.url('/operasional/form_c/produksi/margarin/put'),
          data: d
        }),
      delete: (d) =>
        $.ajax({
          method: 'delete',
          url: baseUrl.url('/operasional/form_c/produksi/margarin/delete'),
          data: d
        }),
    },
    $: {
      modal: {
        tambah: modsel('formMargarin', 'tambah'),
        ubah: modsel('formMargarin', 'ubah'),
        hapus: modsel('formMargarin', 'hapus')
      },
      table: tbsel('formMargarin'),
      search: scsel()
    },
    ...
    load: () => {
      ...
      if (history.pushState) {
        history.pushState(
          null,
          null,
          baseUrl.url(`/operasional/form_c/produksi/margarin${params}`)
        );
      }
          ...
          r.data.forEach((item, index) => {
            obj.$.table.find(tbysel('dataWrapper', true)).append(`
              <tr>
                ...
                <td>${item.tipe_proses_margarin != null ? item.tipe_proses_margarin.tipe_proses_margarin : '-'}</td>
                <td>
                  <a href="#" class="badge badge-warning" data-toggle="modal" data-target=".modal[data-entity='formMargarin'][data-method='ubah']" data-id="${item.id}">Ubah</a>
                  <a href="#" class="badge badge-danger" data-toggle="modal" data-target=".modal[data-entity='formMargarin'][data-method='hapus']" data-id="${item.id}">Hapus</a>
                </td>
              </tr>
            `)}
          );  
    ...
    responsiveContract: () => {
      ...
      obj.$.modal.tambah.on('show.bs.modal', (e) => {
        ...
        obj.rel.tipeProsesMargarin.ajax.search()
          .fail((r) => console.log(r))
          .done((r) => {
            console.log(r);
            $(e.currentTarget).find('form').find('[name="tipe_proses_margarin_id"]').empty().append('<option value="null">-- Pilih Tipe Proses Margarin --</option>');
            r.data.forEach((item, index) => $(e.currentTarget).find('form').find('[name="tipe_proses_margarin_id"]').append(`
              <option value="${item.id}">${item.tipe_proses_margarin}</option>
            `))
          });
      });
        ...
        obj.ajax.get($(e.relatedTarget).data('id'))
          ...
          .done((r) => {
            ...
            obj.rel.tipeProsesMargarin.ajax.search()
              ...
              .done((re) => {
                console.log(re);
                $(e.currentTarget).find('form').find('[name="tipe_proses_margarin_id"]').empty().append('<option>-- Pilih Tipe Proses Margarin --</option>');
                re.data.forEach((item, index) => $(e.currentTarget).find('form').find('[name="tipe_proses_margarin_id"]').append(`
                  <option value="${item.id}">${item.tipe_proses_margarin}</option>
                `));
                $(e.currentTarget).find('form').find('[name="tipe_proses_margarin_id"]').val(r.data.tipe_proses_margarin_id);
              });
