  ...
  let obj = {
    ...
    ajax: {
      get: (id) =>
        $.ajax({
          method: 'get',
          url: baseUrl.url('/operasional/form_c/atribut_karyawan/get'),
          data: {id: id}
        }),
      cabangHarian: (d) =>
        $.ajax({
          method: 'get',
          url: baseUrl.url('/operasional/form_c/atribut_karyawan/cabang_harian'),
          data: d
        }),
      post: (d) =>
        $.ajax({
          method: 'post',
          url: baseUrl.url('/operasional/form_c/atribut_karyawan/post'),
          data: d
        }),
      put: (d) =>
        $.ajax({
          method: 'put',
          url: baseUrl.url('/operasional/form_c/atribut_karyawan/put'),
          data: d
        }),
      delete: (d) =>
        $.ajax({
          method: 'delete',
          url: baseUrl.url('/operasional/form_c/atribut_karyawan/delete'),
          data: d
        }),
    },
    $: {
      modal: {
        tambah: modsel('formAtributKaryawan', 'tambah'),
        ubah: modsel('formAtributKaryawan', 'ubah'),
        hapus: modsel('formAtributKaryawan', 'hapus')
      },
      table: tbsel('formAtributKaryawan'),
      search: scsel()
    },
    ...
    load: () => {
      ...
      if (history.pushState) {
        history.pushState(
          null,
          null,
          baseUrl.url(`/operasional/form_c/atribut_karyawan${params}`)
        );
      }
          ...
          r.data.forEach((item, index) => {
            obj.$.table.find(tbysel('dataWrapper', true)).append(`
              <tr>
                ...
                <td>${item.jam}</td>
                <td>
                  <a href="${baseUrl.url('/operasional/form_c/atribut_karyawan/detail/')}${item.id}" class="badge badge-info">Lihat Detail</a>
                  <a href="#" class="badge badge-warning" data-toggle="modal" data-target=".modal[data-entity='formAtributKaryawan'][data-method='ubah']" data-id="${item.id}">Ubah</a>
                  <a href="#" class="badge badge-danger" data-toggle="modal" data-target=".modal[data-entity='formAtributKaryawan'][data-method='hapus']" data-id="${item.id}">Hapus</a>
                </td>
              </tr>
            `)}
          );  
    ...
    responsiveContract: () => {
      ...
      obj.$.modal.tambah.on('show.bs.modal', (e) => {
        ...
        obj.rel.aktivitasKaryawan.ajax.search()
          .fail((r) => console.log(r))
          .done((r) => {
            console.log(r);
            $(e.currentTarget).find('form').find('[name="aktivitas_karyawan_id"]').empty().append('<option value="null">-- Pilih Aktivitas Karyawan --</option>');
            r.data.forEach((item, index) => $(e.currentTarget).find('form').find('[name="aktivitas_karyawan_id"]').append(`
              <option value="${item.id}">${item.aktivitas_karyawan}</option>
            `))
          });
      });
        ...
        obj.ajax.get($(e.relatedTarget).data('id'))
          ...
          .done((r) => {
            ...
            obj.rel.aktivitasKaryawan.ajax.search()
              ...
              .done((re) => {
                console.log(re);
                $(e.currentTarget).find('form').find('[name="aktivitas_karyawan_id"]').empty().append('<option>-- Pilih Aktivitas Karyawan --</option>');
                re.data.forEach((item, index) => $(e.currentTarget).find('form').find('[name="aktivitas_karyawan_id"]').append(`
                  <option value="${item.id}">${item.aktivitas_karyawan}</option>
                `));
                $(e.currentTarget).find('form').find('[name="aktivitas_karyawan_id"]').val(r.data.aktivitas_karyawan_id);
              });
            obj.rel.atributKaryawan.ajax.search()
              .fail((re) => console.log(re))
              .done((re) => {
                console.log(re);
                r.data.d.forEach((item, index) => {
                  re.data.forEach((it, id) => {
                    $(e.currentTarget).find(`[name="form_atribut_karyawan_d_id[${it.id}]"]`).val(item.id);
                    $(e.currentTarget).find(`[name="parameter_atribut_karyawan_id[${it.id}]"][value="${item.parameter_atribut_karyawan_id}"]`).attr('checked', true);
                  });
                });
              })
